...
def init_agent(count=1):
    for i in range(count):
        init_speed = random.uniform(-0.25, 1.25)
        init_chloroplast = random.uniform(-1, 1)
        init_color = speed_to_color(init_speed, max_speed=settings.max_speed,  chloroplast=init_chloroplast)
        agent = Agent(x=random.randint(0, width), y=random.randint(0, height), speed=init_speed, color=init_color, chloroplast=init_chloroplast)
